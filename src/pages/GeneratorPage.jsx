...
const GeneratorPage = () => {
  ...
  const [generatedPrompt, setGeneratedPrompt] = useState(
    "Pilih jenis halaman terlebih dahulu...",
  );
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTimeout(() => {
          ...
          setGeneratedPrompt(
            "Lengkapi formulir di sebelah kiri untuk meracik prompt Anda...",
          );
        }, 200);
      }
    }, 24);
  ...
  const validateLP = () => {
    ...
    if (
      ...
      valid = false;
    if (d.category_base === "custom" && !d.category_custom.trim())
      valid = false;
  ...
  const validateWeb = () => {
    const d = webData;
    let valid = true;
    if (!d.web_name || !d.web_desc || !d.web_design_style || !d.web_brand_color)
      valid = false;
    ...
    if (d.web_brand_color === "custom" && !d.web_color_custom.trim())
      valid = false;
    return valid;
  };
  ...
  const validateBio = () => {
    ...
    if (
      ...
      valid = false;
    if (d.bio_brand_color === "custom" && !d.bio_color_custom.trim())
      valid = false;
    return valid;
  };
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      let prompt = "";
      if (currentMode === "landing-page")
        prompt = generateLandingPagePrompt(lpData);
      else if (currentMode === "website")
        prompt = generateWebsitePrompt(webData);
      else if (currentMode === "link-bio")
        prompt = generateLinkBioPrompt(bioData);
  ...
  const copyPrompt = () => {
    ...
    if (
      text.includes("Lengkapi formulir") ||
      text.includes("Sedang meracik") ||
      text.includes("Pilih jenis")
    )
      return;
  ...
  const confirmReset = () => {
    ...
    if (currentMode === "landing-page") {
      setLpData(initialLpData);
    } else if (currentMode === "website") {
      setWebData(initialWebData);
    } else if (currentMode === "link-bio") {
  ...
  const inputClass =
    "w-full rounded-xl px-4 py-3.5 text-sm outline-none transition-all duration-200 bg-white border border-slate-200 text-slate-800 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 dark:bg-[#030712]/60 dark:border-gray-800 dark:text-slate-100 dark:focus:border-orange-500";
  ...
  return (
    <div className="min-h-screen pb-20 selection:bg-orange-500 selection:text-white font-sans bg-slate-50 text-slate-900 dark:bg-[#0B0F19] dark:text-slate-200 transition-colors duration-300">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                  {!currentMode && (
                    <>
                      Web<span className="text-orange-500">Engine</span>
                    </>
                  )}
                  ...
                  {currentMode === "website" && (
                    <>
                      Website<span className="text-orange-500"> Engine</span>
                    </>
                  )}
                  {currentMode === "link-bio" && (
                    <>
                      Link Bio<span className="text-orange-500"> Engine</span>
                    </>
                  )}
                </h1>
            ...
            <div className="flex items-center gap-2">
              {currentMode && (
                <button
                  onClick={goBack}
                  className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:text-orange-500 transition-all cursor-pointer"
                  title="Kembali"
                >
                  <svg
                    className="w-5 h-5"
              ...
              <button
                onClick={toggleTheme}
                className="cursor-pointer p-2.5 rounded-xl border-2 transition-all duration-300 focus:outline-none bg-orange-50 border-orange-200 text-orange-600 hover:bg-orange-100 dark:bg-white/5 dark:border-orange-500/50 dark:text-orange-500 dark:hover:bg-orange-500 dark:hover:text-white"
              >
                <svg
              ...
              <button
                ...
                className="hidden sm:block cursor-pointer p-2.5 rounded-xl border-2 transition-all duration-300 focus:outline-none bg-red-50 border-red-200 text-red-600 hover:bg-red-100 dark:bg-white/5 dark:border-red-900/50 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white"
                title="Keluar"
              >
                <svg
                  className="w-5 h-5"
          ...
          <div className="animate-[fadeIn_0.5s_ease-out]">
            <div className="text-center max-w-4xl mx-auto mb-16 space-y-6 relative z-10">
              ...
              <p className="text-lg md:text-xl leading-relaxed max-w-2xl mx-auto font-light text-slate-600 dark:text-slate-400">
                Pilih jenis halaman yang ingin Anda buat, kami bantu susun
                struktur konten yang profesional dan menjual.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              {/* Card: Landing Page */}
              <button
                onClick={() => startTransition("landing-page")}
                className="selection-card group text-left glass-card rounded-3xl p-8 relative overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-xl border border-slate-200/50 dark:border-white/10"
              >
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                  Buat sales page yang persuasif dengan framework copywriting
                  terbukti konversi.
                </p>
              </button>
              ...
              {/* Card: Website */}
              <button
                onClick={() => startTransition("website")}
                className="selection-card group text-left glass-card rounded-3xl p-8 relative overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-xl border border-slate-200/50 dark:border-white/10"
              >
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                  Susun Company Profile atau Portfolio yang profesional dan
                  kredibel.
                </p>
              </button>
              ...
              {/* Card: Link Bio */}
              <button
                onClick={() => startTransition("link-bio")}
                className="selection-card group text-left glass-card rounded-3xl p-8 relative overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-xl border border-slate-200/50 dark:border-white/10"
              >
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                  Desain micro-site link in bio yang modern dan eye-catching.
                </p>
              </button>
            </div>
          </div>
          ...
          <div className="animate-[fadeIn_0.5s_ease-out]">
            ...
            <div className="text-center max-w-4xl mx-auto mb-16 space-y-6 relative z-10">
              ...
              {currentMode === "link-bio" && (
                <>
                  <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight leading-[1.15] text-slate-900 dark:text-white">
                    Bio Page yang Bikin Pengunjung{" "}
                    <span className="text-gradient">Klik Terus</span>
                  </h1>
                  <p className="text-lg md:text-xl leading-relaxed max-w-2xl mx-auto font-light text-slate-600 dark:text-slate-400">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <section className="lg:col-span-7 animate-fade-in">
                  ...
                  <div className="space-y-8">
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        <span className="step-badge w-10 h-10 rounded-xl flex items-center justify-center text-lg font-bold">
                          1
                        </span>
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Tentukan struktur psikologi dan gaya bahasa
                            copywriting.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Apa yang Anda jual dan apa goal utamanya?
                          </p>
                        </div>
                      </div>
                    ...
                    <div className={cardClass}>
                        ...
                        <div className="space-y-2">
                          ...
                          {lpData.target_base === "custom" && (
                            <input
                              type="text"
                              id="target_custom"
                              value={lpData.target_custom}
                              onChange={handleLp}
                              placeholder="Siapa spesifik target market?"
                              className={`${inputClass} mt-3`}
                            />
                          )}
                        </div>
                      ...
                      <div className="space-y-2 mt-6">
                        ...
                        <textarea
                          id="pain_points"
                          rows="3"
                          value={lpData.pain_points}
                          onChange={handleLp}
                          placeholder="Apa ketakutan/masalah utama target Anda?"
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Penawaran dan copywriting utama.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                        ...
                        <div className="space-y-2">
                          <label className={labelClass}>
                            Objections / Alasan Ragu
                          </label>
                          <textarea
                            id="objections"
                            rows="3"
                            value={lpData.objections}
                            onChange={handleLp}
                            placeholder="Alasan terbesar target Anda TIDAK jadi beli?"
                            className={inputClass}
                          ></textarea>
                        </div>
                        {/* Bonus toggle */}
                        <div className="space-y-3">
                          <label className="flex items-center gap-3 cursor-pointer">
                            <input
                              type="checkbox"
                              id="has_bonus"
                              checked={lpData.has_bonus}
                              onChange={handleLp}
                              className="w-5 h-5 rounded border-slate-300 text-orange-500 focus:ring-orange-500"
                            />
                            <span className={labelClass}>
                              Ada Bonus / Value Stack?
                            </span>
                          </label>
                          {lpData.has_bonus && (
                            <textarea
                              id="bonus_details"
                              rows="3"
                              value={lpData.bonus_details}
                              onChange={handleLp}
                              placeholder="Detail bonus yang ditawarkan..."
                              className={inputClass}
                            ></textarea>
                          )}
                        </div>
                          ...
                          <div className="space-y-2">
                            ...
                            {lpData.cta_base === "custom" && (
                              <input
                                type="text"
                                id="cta_custom"
                                value={lpData.cta_custom}
                                onChange={handleLp}
                                placeholder="Teks tombol custom..."
                                className={`${inputClass} mt-3`}
                              />
                            )}
                          </div>
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Pilih section apa saja yang ingin ditampilkan.
                          </p>
                        </div>
                      </div>
                        ...
                        {lpSectionItems.map(([val, lbl]) => (
                          <div key={val} className="relative group">
                            <input
                              ...
                              id={`sec_${val}`}
                              value={val}
                              checked={lpData.sections.includes(val)}
                              onChange={handleLp}
                              className="hidden section-checkbox"
                              data-group="sections"
                            />
                            <label
                              htmlFor={`sec_${val}`}
                              className="flex flex-col items-center justify-center text-center text-xs font-bold h-14 rounded-xl cursor-pointer select-none"
                            >
                              {lbl}
                            </label>
                          </div>
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Warna, hero type, dan gaya visual.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Informasi dasar brand dan website.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                          ...
                          <div className="space-y-2">
                            ...
                            {webData.web_type === "custom" && (
                              <input
                                type="text"
                                id="web_type_custom"
                                value={webData.web_type_custom}
                                onChange={handleWeb}
                                placeholder="Tipe website custom..."
                                className={`${inputClass} mt-3`}
                              />
                            )}
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              id="web_tagline"
                              value={webData.web_tagline}
                              onChange={handleWeb}
                              placeholder="Tagline brand Anda"
                              className={inputClass}
                            />
                          </div>
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              id="web_seo"
                              value={webData.web_seo}
                              onChange={handleWeb}
                              placeholder="Keyword utama (pisahkan koma)"
                              className={inputClass}
                            />
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <textarea
                            id="web_desc"
                            rows="4"
                            value={webData.web_desc}
                            onChange={handleWeb}
                            placeholder="Jelaskan brand / bisnis Anda secara detail..."
                            className={inputClass}
                          ></textarea>
                        </div>
                      </div>
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Deskripsi, masalah, dan solusi brand.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            <textarea
                              id="web_problem"
                              rows="3"
                              value={webData.web_problem}
                              onChange={handleWeb}
                              placeholder="Masalah apa yang brand Anda selesaikan?"
                              className={inputClass}
                            ></textarea>
                          </div>
                          <div className="space-y-2">
                            ...
                            <textarea
                              id="web_solution"
                              rows="3"
                              value={webData.web_solution}
                              onChange={handleWeb}
                              placeholder="Solusi apa yang Anda tawarkan?"
                              className={inputClass}
                            ></textarea>
                          </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            <FlatSelect
                              id="web_hero_style"
                              value={webData.web_hero_style}
                              onChange={handleWeb}
                              options={webHeroStyleOptions}
                              className={selectClass}
                            />
                          </div>
                          <div className="space-y-2">
                            ...
                            <FlatSelect
                              id="web_page_structure"
                              value={webData.web_page_structure}
                              onChange={handleWeb}
                              options={webPageStructureOptions}
                              className={selectClass}
                            />
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                          ...
                          {webData.web_target === "custom" && (
                            <input
                              type="text"
                              id="web_target_custom"
                              value={webData.web_target_custom}
                              onChange={handleWeb}
                              placeholder="Target pengunjung custom..."
                              className={`${inputClass} mt-3`}
                            />
                          )}
                        </div>
                        <div className="space-y-2">
                          ...
                          <FlatSelect
                            id="web_tone"
                            value={webData.web_tone}
                            onChange={handleWeb}
                            options={webToneOptions}
                            className={selectClass}
                          />
                          {webData.web_tone === "custom" && (
                            <input
                              type="text"
                              id="web_tone_custom"
                              value={webData.web_tone_custom}
                              onChange={handleWeb}
                              placeholder="Tone custom..."
                              className={`${inputClass} mt-3`}
                            />
                          )}
                        </div>
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            USP, tahun berdiri, dan link sosial media.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                        <div className="space-y-2">
                          ...
                          <textarea
                            id="web_usp"
                            rows="2"
                            value={webData.web_usp}
                            onChange={handleWeb}
                            placeholder="Apa keunikan brand Anda?"
                            className={inputClass}
                          ></textarea>
                        </div>
                      ...
                      <div className="space-y-5">
                        <div className="space-y-2">
                          ...
                          <textarea
                            id="web_services"
                            rows="3"
                            value={webData.web_services}
                            onChange={handleWeb}
                            placeholder="List layanan / produk utama..."
                            className={inputClass}
                          ></textarea>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Navigasi, sections, dan elemen fungsional.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-6">
                            ...
                            {webNavStyles.map((ns) => (
                              <div key={ns} className="relative">
                                <input
                                  ...
                                  id={`wnav_${ns}`}
                                  value={ns}
                                  checked={webData.web_nav_style === ns}
                                  onChange={handleWeb}
                                  className="hidden peer"
                                />
                                <label
                              ...
                              <div key={item.id} className="relative group">
                                <input
                                  ...
                                  checked={
                                    item.locked
                                      ? true
                                      : webData.web_sections.includes(item.id)
                                  }
                            ...
                            {webFunctionalItems.map((item) => (
                              <div key={item.id} className="relative group">
                                <input
                                  ...
                                  value={item.id}
                                  onChange={handleWeb}
                                  checked={webData.web_functionals.includes(
                                    item.id,
                                  )}
                                  className="hidden section-checkbox-web peer"
                      ...
                      <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            {webData.web_brand_color === "custom" && (
                              <input
                                type="text"
                                id="web_color_custom"
                                value={webData.web_color_custom}
                                onChange={handleWeb}
                                placeholder="#Hex atau Nama Warna"
                                className={`${inputClass} mt-3`}
                              />
                          ...
                          <div className="space-y-2">
                            ...
                            <FlatSelect
                              id="web_theme_override"
                              value={webData.web_theme_override}
                              onChange={handleWeb}
                              options={shortThemeOverrideOptions}
                              className={selectClass}
                            />
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <GroupedSelect
                            id="web_design_style"
                            value={webData.web_design_style}
                            onChange={handleWeb}
                            options={designStyleOptions}
                            className={selectClass}
                          />
                        ...
                        {webPlatforms.map((p) => (
                          <div key={p} className="relative">
                            <input
                              ...
                              id={`web_plt_${p}`}
                              value={p}
                              checked={webData.web_platform === p}
                              onChange={handleWeb}
                              className="hidden peer"
                            />
                            <label
                  ...
                  <div className="space-y-8">
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <textarea
                            id="bio_desc"
                            rows="3"
                            value={bioData.bio_desc}
                            onChange={handleBio}
                            placeholder="Deskripsikan diri Anda dalam 1-2 kalimat..."
                            className={inputClass}
                          ></textarea>
                        </div>
                      </div>
                    ...
                    <div className={`${cardClass} border-dashed`}>
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Link sosial media yang ditampilkan.
                          </p>
                        </div>
                      </div>
                      <p className="text-[10px] text-slate-500 mb-4 bg-slate-100 dark:bg-white/5 p-3 rounded-lg">
                    ...
                    <div className={cardClass}>
                      ...
                      <div className="space-y-5">
                        <div className="space-y-2">
                          ...
                          <FlatSelect
                            id="bio_layout"
                            value={bioData.bio_layout}
                            onChange={handleBio}
                            options={bioLayoutOptions}
                            className={selectClass}
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          <input
                            type="text"
                            id="bio_main_link"
                            value={bioData.bio_main_link}
                            onChange={handleBio}
                            placeholder="Link utama yang di-highlight"
                            className={inputClass}
                          />
                        </div>
                        <div className="space-y-2">
                          <label className={labelClass}>
                            List Link <span className="text-orange-500">*</span>
                          </label>
                          <div className="mb-3 p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-dashed border-slate-300 dark:border-white/10 text-[11px] text-slate-500 leading-relaxed">
                            ...
                            <br />
                            Katalog Produk : https://website.com/katalog
                            <br />
                            Admin WhatsApp : https://wa.me/62812xxx
                            <br />
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Foto profil, background, dan tombol.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {bioPhotoShapes.map((shape) => (
                              <div key={shape} className="relative">
                                <input
                                  type="radio"
                                  name="bio_photo_shape"
                                  id={`bps_${shape}`}
                                  value={shape}
                                  checked={bioData.bio_photo_shape === shape}
                                  onChange={handleBio}
                                  className="hidden peer"
                                />
                                <label
                        ...
                        <div className="space-y-2">
                          ...
                          <FlatSelect
                            id="bio_bg_type"
                            value={bioData.bio_bg_type}
                            onChange={handleBio}
                            options={bioBgTypeOptions}
                            className={selectClass}
                          />
                          {(bioData.bio_bg_type === "Image Background" ||
                            bioData.bio_bg_type === "Video Background") && (
                            <input
                              type="text"
                              id="bio_bg_url"
                              value={bioData.bio_bg_url}
                              onChange={handleBio}
                              placeholder="URL gambar / video background"
                              className={`${inputClass} mt-3`}
                            />
                          )}
                        </div>
                            ...
                            {bioBtnShapes.map((shape) => (
                              <div key={shape} className="relative">
                                <input
                                  ...
                                  id={`bbs_${shape}`}
                                  value={shape}
                                  checked={bioData.bio_btn_shape === shape}
                                  onChange={handleBio}
                                  className="hidden peer"
                                />
                                <label
                    ...
                    <div className={cardClass}>
                      <div className="flex items-center gap-4 mb-8">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Warna, tema, gaya desain, animasi, dan font.
                          </p>
                        </div>
                      </div>
                      <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            {bioData.bio_brand_color === "custom" && (
                              <input
                                type="text"
                                id="bio_color_custom"
                                value={bioData.bio_color_custom}
                                onChange={handleBio}
                                placeholder="#Hex atau Nama Warna"
                                className={`${inputClass} mt-3`}
                              />
                          ...
                          <div className="space-y-2">
                            ...
                            <FlatSelect
                              id="bio_theme_override"
                              value={bioData.bio_theme_override}
                              onChange={handleBio}
                              options={shortThemeOverrideOptions}
                              className={selectClass}
                            />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="space-y-2">
                            ...
                            <GroupedSelect
                              id="bio_design_style"
                              value={bioData.bio_design_style}
                              onChange={handleBio}
                              options={designStyleOptions}
                              className={selectClass}
                            />
                          ...
                          <div className="space-y-2">
                            ...
                            <FlatSelect
                              id="bio_anim"
                              value={bioData.bio_anim}
                              onChange={handleBio}
                              options={bioAnimOptions}
                              className={selectClass}
                            />
                          </div>
                        </div>
                        <div className="space-y-2">
                          <label className={labelClass}>
                            Jenis Font (Typography){" "}
                            <span className="text-orange-500">*</span>
                          </label>
                          <FlatSelect
                            id="bio_font"
                            value={bioData.bio_font}
                            onChange={handleBio}
                            options={bioFontOptions}
                            className={selectClass}
                          />
                        </div>
                      </div>
                    ...
                    <div className={`${cardClass} border-dashed`}>
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                            Featured Content (Opsional)
                          </h2>
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Widget dan konten tambahan.
                          </p>
                        </div>
                      </div>
                      <p className="text-[10px] text-slate-500 mb-4 bg-slate-100 dark:bg-white/5 p-3 rounded-lg">
                      ...
                      <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              id="bio_countdown"
                              value={bioData.bio_countdown}
                              onChange={handleBio}
                              placeholder="Contoh: Flash Sale berakhir!"
                              className={inputClass}
                            />
                          </div>
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              id="bio_music"
                              value={bioData.bio_music}
                              onChange={handleBio}
                              placeholder="URL Spotify/Apple Music"
                              className={inputClass}
                            />
                          </div>
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              id="bio_donation"
                              value={bioData.bio_donation}
                              onChange={handleBio}
                              placeholder="Teks untuk tombol donasi"
                              className={inputClass}
                            />
                          </div>
                        </div>
                        <div className="mt-4 relative group">
                          <input
                            ...
                            id="bio_contact_form"
                            value="Contact Form"
                            checked={bioData.bio_contact_form}
                            onChange={handleBio}
                            className="hidden section-checkbox-bio"
                          />
                          <label
                            htmlFor="bio_contact_form"
                            className="flex flex-col items-center justify-center text-center text-xs font-bold h-12 rounded-xl cursor-pointer select-none"
                          >
                            Tambahkan Form Kontak Langsung
                          </label>
                        </div>
                      </div>
                    </div>
                        ...
                        {bioPlatforms.map((p) => (
                          <div key={p} className="relative">
                            <input
                              ...
                              id={`bio_plt_${p}`}
                              value={p}
                              checked={bioData.bio_platform === p}
                              onChange={handleBio}
                              className="hidden peer"
                            />
                            <label
                ...
                {showValidationMsg && (
                  <div
                    id="validation-msg"
                    className="bg-red-500/10 border border-red-500/50 text-red-600 dark:text-red-200 px-6 py-4 rounded-2xl text-sm font-medium flex items-center gap-3 backdrop-blur-sm"
                  >
                    <svg
                    ...
                    Mohon lengkapi semua kolom bertanda (Wajib) agar prompt
                    akurat.
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-4">
                  <button
                    onClick={handleReset}
                    className="col-span-1 border border-slate-300 text-slate-500 hover:bg-slate-50 dark:border-[#2d294a] dark:text-slate-400 dark:hover:text-white dark:hover:bg-white/5 font-bold py-5 rounded-3xl transition-all cursor-pointer"
                  >
                    Reset
                  </button>
                  <button
                    ...
                    className="col-span-2 btn-gradient text-white font-extrabold py-5 rounded-3xl shadow-xl hover:scale-[1.02] active:scale-95 text-lg uppercase tracking-widest flex items-center justify-center gap-2 group cursor-pointer disabled:opacity-80 disabled:cursor-not-allowed"
                  >
                    {isGenerating ? (
                      <>
                        <svg
                ...
                <div className="rounded-[2.5rem] p-1 shadow-2xl flex flex-col h-[750px] border transition-all duration-300 border-white/40 bg-white dark:border-white/10 dark:bg-gray-900/50 backdrop-blur-xl">
                  ...
                  <div className="flex items-center justify-between px-8 py-6 border-b rounded-t-[2.3rem] border-slate-200 bg-slate-50 dark:border-white/5 dark:bg-white/5">
                    <div className="flex flex-col">
                      <span className="text-orange-500 text-[10px] font-bold uppercase tracking-[0.2em] mb-1">
                        Result
                      </span>
                    ...
                    <div className="flex flex-col items-center gap-3">
                      <button
                        onClick={copyPrompt}
                        className="bg-orange-500 text-white border border-transparent hover:shadow-[0_0_20px_rgba(249,115,22,0.6)] hover:scale-105 transition-all duration-300 text-xs px-5 py-2.5 rounded-2xl font-bold flex items-center gap-2 cursor-pointer"
                      >
                        <svg
                          className="h-4 w-4"
                          fill="none"
                  ...
                  <div
                    ref={outputRef}
                    id="output_container"
                    className="flex-1 overflow-auto p-8 relative rounded-b-[2rem] bg-white dark:bg-[#030509]"
                  >
                    <pre className="text-[13px] whitespace-pre-wrap leading-relaxed font-mono selection:bg-orange-500 selection:text-white text-slate-600 dark:text-slate-400">
                      {generatedPrompt}
                      {isStreaming && (
                        <span className="inline-block w-[2px] h-[1em] bg-orange-500 align-middle ml-0.5 animate-pulse" />
                      )}
                    </pre>
                  </div>
                  {/* Bottom CTA */}
                  <div className="w-full p-4 bg-white dark:bg-transparent rounded-b-[2.3rem]">
                    <button
                      onClick={goToGPT}
                      className="bg-orange-500 text-white border border-transparent hover:shadow-[0_0_20px_rgba(249,115,22,0.6)] hover:scale-105 transition-all duration-300 text-xs px-5 py-2.5 rounded-2xl font-bold w-full flex items-center gap-2 justify-center cursor-pointer"
                    >
                      <svg
                        className="h-4 w-4"
                        fill="none"
      ...
      <div className="fixed bottom-6 right-6 z-50 sm:hidden">
        <button
          onClick={handleLogout}
          className="cursor-pointer p-2.5 rounded-xl border-2 transition-all duration-300 focus:outline-none bg-red-50 border-red-200 text-red-600 hover:bg-red-100 dark:bg-white/5 dark:border-red-900/50 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white"
          title="Keluar"
        >
          <svg
            className="w-5 h-5"
